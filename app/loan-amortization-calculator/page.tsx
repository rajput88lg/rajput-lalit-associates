...
import LoanAmortizationCalculatorTool from "@/components/tools/LoanAmortizationCalculatorTool";

const SLUG = "loan-amortization-calculator";
const PAGE_URL = `https://www.rajputlalitassociates.in/${SLUG}`;
const OG_IMAGE = `/og/${SLUG}.png`;

const TITLE = "Loan Amortization Calculator (Free) — Monthly Payment & Schedule | Rajput Lalit & Associates";
const DESCRIPTION = "Free loan amortization calculator — enter loan amount, interest rate and term to see your monthly payment, total interest and a year-wise repayment schedule. Works for any currency.";

export const metadata: Metadata = {
  title: TITLE,
  description: DESCRIPTION,
  keywords: ["loan amortization calculator", "mortgage calculator online free", "loan repayment schedule calculator", "amortization schedule calculator"],
  ...
  openGraph: {
    ...
    locale: "en_US",
    type: "website",
    images: [{ url: OG_IMAGE, width: 1200, height: 630, alt: "Loan Amortization Calculator — Rajput Lalit & Associates" }],
  },
  twitter: {
...
const faqs = [
  { question: "How is the monthly loan payment calculated?", answer: "Using the standard reducing-balance formula: EMI = P x r x (1+r)^n / ((1+r)^n - 1), where P is the loan amount, r is the monthly interest rate, and n is the number of months. This is the same formula banks use worldwide for mortgages and term loans." },
  { question: "Why does most of my early payment go toward interest?", answer: "Because interest is calculated on the outstanding balance, which is highest at the start of the loan. As the balance falls, more of each fixed payment goes toward principal — this is normal amortization behaviour, not an error." },
  { question: "Does this work for mortgages as well as personal loans?", answer: "Yes — the reducing-balance formula is the same for mortgages, auto loans, personal loans and business loans; only the typical amount, rate and term differ." },
  { question: "Is this specific to any one country's currency?", answer: "No — figures are shown in a generic currency symbol since the maths works identically in any currency. For India-specific home loan or business loan tools, see our dedicated EMI calculators." },
  { question: "Is this calculator free to use?", answer: "Yes, completely free with no sign-up or limits." }
];
  ...
  return (
    <>
      ...
      <main>
        <section className="relative overflow-hidden bg-gradient-to-br from-[#001d40] via-[#002b5c] to-[#06477f] text-white print:hidden">
          ...
          <div className="relative max-w-5xl mx-auto px-6 py-20 md:py-24 text-center">
            ...
            <p className="hero-in hero-in-3 mt-6 text-lg md:text-xl text-blue-100 leading-8 max-w-3xl mx-auto">
              Work out the monthly payment and full repayment schedule for any loan or mortgage, worldwide.
            </p>
        ...
        <div className="print:hidden">
          <Breadcrumb current="Loan Amortization Calculator" />
        </div>
