...
const SLUG = "world-time-zone-converter";
const PAGE_URL = `https://www.rajputlalitassociates.in/${SLUG}`;
const OG_IMAGE = `/og/${SLUG}.png`;

const TITLE = "World Time Zone Converter (Free) — Convert Time Between Cities | Rajput Lalit & Associates";
const DESCRIPTION = "Free world time zone converter — convert a date and time between any two time zones, with daylight saving automatically accounted for. No sign-up needed.";

export const metadata: Metadata = {
  title: TITLE,
  description: DESCRIPTION,
  keywords: ["time zone converter", "world clock converter", "time zone calculator online free", "convert time between time zones", "IST to EST converter"],
  ...
  openGraph: {
    ...
    locale: "en_US",
    type: "website",
    images: [{ url: OG_IMAGE, width: 1200, height: 630, alt: "World Time Zone Converter — Rajput Lalit & Associates" }],
  },
  twitter: {
...
const faqs = [
  { question: "Does this calculator account for daylight saving time (DST)?", answer: "Yes — it uses each location's actual time zone rules for the specific date you enter, so DST transitions are handled automatically and correctly." },
  { question: "How do I convert IST to US Eastern Time?", answer: "Select India (Asia/Kolkata) as the origin zone and USA East (America/New_York) as the destination zone, enter your date and time, and the converted time appears instantly." },
  { question: "Why does the date sometimes change when I convert time zones?", answer: "Because of the time difference, converting a time near midnight can push the result into the previous or next calendar day — the calculator clearly shows this as a day change." },
  { question: "Is this useful for scheduling calls with clients abroad?", answer: "Yes — it's especially useful for NRIs and businesses coordinating meetings, filings or consultations across different countries and time zones." },
  { question: "Is this calculator free to use?", answer: "Yes, completely free with no sign-up or limits, and it supports major time zones worldwide." }
];
  ...
  return (
    <>
      ...
      <main>
        ...
        <div className="print:hidden">
          <Breadcrumb current="World Time Zone Converter" />
        </div>
